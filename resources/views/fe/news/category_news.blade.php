    ...
    <section id="news">
        <div class="container">
            ...
            <div class="d-flex justify-content-center py-4">
                <h2>- {{ Str::upper($categoryName->news_category) }} -</h2>
            </div>
            <div class="row">
                <div class="col-md-8">
                    <div class="row">
                        ...
                        <div class="col-md-4 w-50">  
                            <div class="card mb-4">
                                ...
                                <a href="{{ route('news.details', $post->id) }}"><img src="{{ url('images/news/' . $post->image) }}" class="card-img-top" alt="News Image"></a>
                                </div>
                                <div class="card-body card-shadow p-3">
                                    <h5 class="card-title"><a href="{{ route('news.details', $post->id) }}">{{ $post->title }}</a></h5>
                                    <small><i class="fas fa-calendar-alt"></i> {{ Carbon\Carbon::parse($post->created_at)->diffForHumans() }}</small>
                                    <p class="card-text">{!! Str::limit($post->content, 100) !!}</p>
                                    <a class="d-flex align-items-center" href="{{ route('news.details', $post->id) }}">Read more<i class="fas fa-long-arrow-alt-right ms-2"></i></a>
                                </div>
                            </div>
                        </div>
                        @endforeach
